...
export async function main(internalStorageUnit: IStorageUnit, smilUrl: string, thisSos: FrontApplet) {
	// allow infinite loop function in this iteration of playlist
...
async function startSmil(smilUrl: string) {
	...
	while (true) {
		try {
			...
			debug(`One smil iteration started ${startVersion}`);
			await main(internalStorageUnit, smilUrl, sos);
			const finishVersion = playlist.getPlaylistVersion();
			debug(`One smil iteration finished ${finishVersion}`);
